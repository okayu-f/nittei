...
const Calender: React.FC<CalenderProps> = ({ onCalenderClick }) => {
  ...
  const [selectedDate, setSelectedDate] = useState(getDate(today));
  ...
  return (
    <>
      ...
      {Array.from({ length: 5 }, (_, index) => (
        <Week key={index} dates={dates.slice(index * 7, (index + 1) * 7)} year={year} month={month} selectedDate={selectedDate} onDayClick={handleClick} />
      ))}
    </>
  );
};
...
const Week: React.FC<WeekProps> = ({ dates, year, month, selectedDate, onDayClick }) => {
  return (
    <Grid container columns={7} spacing={0}>
      {dates.map((date, index) => (
        <Grid item xs={1} key={index}>
          <Day date={date} year={year} month={month} selectedDate={selectedDate} onDayClick={() => onDayClick(date, year, month)} />
        </Grid>
      ))}
    </Grid>
  );
